...
# Define the date 7 days ago in YYYYMMDD format
seven_days_ago=$(date -d "7 days ago" +%Y%m%d)

# Create a list of directories in the last 7 days (YYYY/MM/DD format)
directories_to_check=($(find "$base_directory" -type d -newermt "$seven_days_ago" -printf "%Y/%m/%d\n" | sort -u))
